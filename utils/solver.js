...
const SolverNoProps = require("../sheep-solver/stage1-noprops");
...
const findSolutionNoProps = (mapData, t = [30, 30, 30]) => {
    return new Promise((resolve) => {
        console.log("启动无道具模式");
        const solverNoProps = new SolverNoProps(mapData, t[0]);
        let solution = solverNoProps.findSolution();
        resolve(solution);
    });
};
...
const startThreads = (mapData, timeout) => {
    const promises = [];
    //promises.push(findSolutionNoProps(mapData, timeout));
...
const waitForSomeTime = async (runningTime,rankTime) => {
    process.stdout.write(`求解线程运行时间: \x1b[32m${runningTime.toFixed(2)}\x1b[0m 秒，`);
    if (runningTime < rankTime) {
      const waitTime = rankTime - runningTime;
      process.stdout.write(`等待 \x1b[32m${waitTime}\x1b[0m 秒\n`);
      process.stdout.write("===================================\n");
      var index = waitTime;
      var timer = setInterval(() => {
        index--;
        process.stdout.write(`请等待: \x1b[32m${index}\x1b[0m 秒\r`);
        if (index === 0) {
          clearInterval(timer);
        }
      }, 1000);
...
const sendSolutionToServer = async (
    ...
    const rankTime = matchPlayInfo.rankTime;
     console.log("rankTime", rankTime);
    await waitForSomeTime(runningTime, matchPlayInfo.waitTime);
